	...
	if(is_null($item))
	{
		...
		switch($_SERVER['REQUEST_METHOD'])
		{
			case "GET": //Return a list of all items
				...
				return api_success(array("items" => $item_list));
				break;

			case "POST": //Import uploaded CSV file
				//Verify uploaded file
				if(!upload_attempted("csv")) return api_failure("No file provided");
				if(!upload_okay("csv")) return api_failure("File upload failed");
				//Parse CSV lines
				$csv = file_get_contents($_FILES['csv']['tmp_name']);
				$csv = explode("\n", $csv);
				//Require at least 2 rows (title and data)
				if(count($csv) < 2) return api_failure("Invalid CSV");
				...
				foreach($csv as $line)
				{
					...
					if(count($line) != 2) return api_failure("Invalid number of columns");
					$name = trim($line[0]);
					//Convert price to fixed point (cent)
					$price = price_to_cents(trim($line[1]));
					//Verify contents
					if(!strlen($name) || $price < 0) return api_failure("Invalid data");
					//Attempt insertion
					$item = array();
					$item["name"] = $name;
					$item["price"] = $price;
					if(!db_insert("item", $item)) return api_dbfailure();
				}
				break;
